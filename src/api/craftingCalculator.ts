...
export class CraftingCalculator {
  ...
  async calculateBestMethod(
    ...
    try {
      ...
      if (eldritchMethod) methods.push(eldritchMethod);
      if (awakenersMethod) methods.push(awakenersMethod);
      if (mavensMethod) methods.push(mavensMethod);
      if (influencedExaltMethod) methods.push(influencedExaltMethod);
   ...
   */
  private async calculateEldritchCrafting(
    desiredMods: DesiredMod[],
    baseItemName: string,
    itemLevel: number,
    itemClass: string,
   ...
   */
  private async calculateAwakenersOrb(
    desiredMods: DesiredMod[],
    baseItemName: string,
    itemLevel: number,
    itemClass: string,
    ...
    const hasInfluencedMods = desiredMods.some(mod =>
      influencedKeywords.some(kw => mod.name.toLowerCase().includes(kw))
    );
   ...
   */
  private async calculateMavensOrb(
    desiredMods: DesiredMod[],
    baseItemName: string,
    itemLevel: number,
    itemClass: string,
    ...
    const hasElevatedGoal = desiredMods.some(mod =>
      influencedKeywords.some(kw => mod.name.toLowerCase().includes(kw))
    );
   ...
   */
  private async calculateInfluencedExalt(
    desiredMods: DesiredMod[],
    baseItemName: string,
    itemLevel: number,
    itemClass: string,
   ...
   */
  private async calculateRecombinatorCrafting(
    desiredMods: DesiredMod[],
    baseItemName: string,
    itemLevel: number,
    itemClass: string,
