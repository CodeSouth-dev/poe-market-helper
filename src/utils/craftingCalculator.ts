...
export function createCraftingMethod(
  ...
  const templates: Partial<Record<CraftingMethodType, { name: string; method: string }>> = {
    [CraftingMethodType.ALT_SPAM]: { name: 'Alteration Spam', method: 'alteration' },
    [CraftingMethodType.ESSENCE]: { name: 'Essence Crafting', method: 'essence' },
    [CraftingMethodType.FOSSIL]: { name: 'Fossil Crafting', method: 'fossil' },
    [CraftingMethodType.CHAOS_SPAM]: { name: 'Chaos Spam', method: 'chaos' },
    [CraftingMethodType.RECOMBINATOR]: { name: 'Recombinator', method: 'harvest' },
    [CraftingMethodType.HARVEST]: { name: 'Harvest Reforge', method: 'harvest' },
    [CraftingMethodType.VEILED_CHAOS]: { name: 'Veiled Chaos', method: 'veiled' },
    [CraftingMethodType.ELDRITCH]: { name: 'Eldritch Crafting', method: 'chaos' },
    [CraftingMethodType.BENCHCRAFT]: { name: 'Benchcraft', method: 'bench' },
  };

  const template = templates[type] || { name: 'Unknown', method: 'chaos' };

  return {
    method: template.method as any,
    name: template.name,
    description: description || `Craft using ${template.name}`,
    probability: 1 / averageAttempts,
    averageCost: 0, // Will be calculated from materials
    currencyUsed: {},
    steps: [],
    expectedAttempts: averageAttempts,
    materials,
    averageAttempts,
  };
}
