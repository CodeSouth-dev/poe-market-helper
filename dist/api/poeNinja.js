...
class PoeNinjaAPI {
     ...
     */
    async getBuildCraftableItems(league, limit = 20) {
        const allItems = [];
        ...
        const buildItems = craftableItems.map(item => {
     ...
     */
    generateCraftingStrategy(item, league) {
        const itemName = item.name.toLowerCase();
        ...
        if (baseType.includes('ring'))
            ...
            itemClass = 'Belt';
        else if (baseType.includes('regalia') || baseType.includes('armour'))
            itemClass = 'Body Armour';
        else if (baseType.includes('helmet') || baseType.includes('helm'))
            itemClass = 'Helmet';
        else if (baseType.includes('glove'))
            itemClass = 'Gloves';
        else if (baseType.includes('boot'))
            itemClass = 'Boots';
        else if (baseType.includes('jewel'))
            itemClass = 'Jewel';
        else if (baseType.includes('axe') || baseType.includes('sword') || baseType.includes('mace'))
            itemClass = 'Weapon';
     ...
     */
    determineCraftingStrategy(item, itemClass, sellPrice) {
        const itemName = item.name.toLowerCase();
        const baseType = (item.baseType || '').toLowerCase();
        const variant = (item.variant || '').toLowerCase();
     ...
     */
    parseBuildsApiResponse(data, league) {
        const builds = [];
        const itemUsageMap = new Map();
        // Parse build entries
        if (data && Array.isArray(data)) {
            data.forEach((buildEntry) => {
                const build = {
     ...
     */
    parseNextDataBuilds(nextData, league) {
